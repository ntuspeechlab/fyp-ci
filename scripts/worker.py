...
worker_status = prom.Enum('worker_status', 'Status of worker',
                          states=['idle', 'running'])
worker_model = prom.Info(
    'worker_model', 'Model which the worker is using to decode')
...
class ServerWebsocket(worker_addon.BaseServerWebsocket):
    ...
    def __init__(self, uri, decoder_pipeline, post_processor, full_post_processor=None):
        if sys.version_info[0] < 3:
            uri = uri + \
                '?%s' % (urllib.urlencode([("model", os.getenv('MODEL_DIR'))]))
        else:
            uri = uri + \
        ...
        worker_addon.BaseServerWebsocket.__init__(
            self, url=uri, heartbeat_freq=10)
        ...
        while self.state in [self.STATE_EOS_RECEIVED, self.STATE_CONNECTED, self.STATE_INITIALIZED, self.STATE_PROCESSING]:
            if time.time() - self.last_decoder_message > SILENCE_TIMEOUT:
                ...
                try:
                    ...
                    logger.warning(
                        "%s: Failed to send error event to master" % (self.request_id))
                self.close()
                return
    ...
    def received_message(self, m):
        eos_m = ""
        if m.is_text:
            eos_m = m.data.decode("utf-8")
        elif m.is_binary:
            eos_m = str(m.data)[2:-1]
        else:
            eos_m = m.data
        ...
        if self.state == self.__class__.STATE_CONNECTED:
            decoded_m = ""
            try:
                decoded_m = m.data.decode("utf-8")
            except:
                decoded_m = str(m.data)[2:-1]

            props = json.loads(decoded_m)
        ...
        elif (eos_m == "keepalive"):
            self.state = self.STATE_IDLE
            # self.decoder_pipeline.clear_buffer()
            logger.info("%s: User sending I-AM-ALIVE message." %
                        (self.request_id))
                ...
                if isinstance(m, ws4py.messaging.BinaryMessage):
                    ...
                    if 'adaptation_state' in props:
                        ...
                        if as_props.get('type', "") == "string+gzip+base64":
                            adaptation_state = zlib.decompress(
                                base64.b64decode(as_props.get('value', '')))
                            logger.info(
                                "%s: Setting adaptation state to user-provided value" % (self.request_id))
                            self.decoder_pipeline.set_adaptation_state(
                                adaptation_state)
                        else:
                        ...
                        logger.warning(
                            "%s: Got JSON message but don't know what to do with it" % (self.request_id))
            else:
    ...
    def finish_request(self):
        ...
        if self.state != self.STATE_FINISHED:
            logger.info(
                "%s: Master disconnected before decoder reached EOS?" % self.request_id)
    ...
    def _on_result(self, result, final):
        try:
            ...
            logger.info("%s: Postprocessing (final=%s) result.." %
                        (self.request_id, final))
    ...
    def _on_full_result(self, full_result_json):
        try:
            ...
            if full_result.get("status", -1) == common.STATUS_SUCCESS:
                ret_full_result = ""
                try:
                    ret_full_result = repr(
                        full_result).decode("unicode-escape")
                except:
                    ret_full_result = repr(full_result)

                logger.debug(u"%s: Before postprocessing: %s" %
                             (self.request_id, ret_full_result))
                try:
                    full_result = yield self.post_process_full(full_result)
                except Exception as e:
                    logger.info('EXCEPTION! in _on_full_result()')
                    logger.info(e)
    ...
    def _on_eos(self, data=None):
        ...
        while self.num_processing_threads > 0:
            logging.debug("Waiting until processing threads finish (%d)" %
                          self.num_processing_threads)
    ...
    def send_adaptation_state(self):
        if hasattr(self.decoder_pipeline, 'get_adaptation_state'):
            ...
            if sys.version_info[0] < 3:
                adaptationvalue = base64.b64encode(
                    zlib.compress(adaptation_state))
            else:
                adaptationvalue = base64.b64encode(
                    zlib.compress(adaptation_state.encode()))

            event = dict(status=common.STATUS_SUCCESS,
                         adaptation_state=dict(id=self.request_id,
                                               value=adaptationvalue.decode(),
                                               type="string+gzip+base64",
                                               time=time.strftime("%Y-%m-%dT%H:%M:%S")))
            ...
            logger.info("%s: Adaptation state not supported by the decoder, not sending it." % (
                self.request_id))
        ...
        if self.post_processor:
            if self.post_processor_lock.acquire(blocking):
                ...
                for text in texts:
                    ...
                    logger.debug("%s: Starting postprocessing: %s" %
                                 (self.request_id, text))
                    text = yield self.post_processor.stdout.read_until(b'\n')
                    text = text.decode("utf-8")
                    text = text.strip()
                    text = text.replace("\\n", "\n")
                    logger.debug("%s: Postprocessing returned: %s" %
                                 (self.request_id, text))
                    result.append(text)
                ...
                logging.debug(
                    "%s: Skipping postprocessing since post-processor already in use" % (self.request_id))
    ...
    def post_process_full(self, full_result):
        if self.full_post_processor:
            if sys.version_info[0] < 3:
                ...
                self.full_post_processor.stdin.write(
                    ("%s\n\n" % json.dumps(full_result)).encode('utf-8'))
            ...
            while True:
                ...
                nl = l
                if sys.version_info[0] >= 3:
                    nl = l.decode("utf-8").strip('\n')
                if not nl:
                    break  # EOF

                logger.debug("%s: full_post_processor returned: %s" %
                             (self.request_id, nl))
                if nl.strip() == "()":
                    break
                if nl.strip() == "":
...
def main_loop(uri, decoder_pipeline, post_processor, full_post_processor=None):
    ...
    while True:
        ws = ServerWebsocket(uri, decoder_pipeline, post_processor,
...
def main():
    ...
    parser.add_argument('-u', '--uri', default="ws://localhost:8888/worker/ws/speech",
                        dest="uri", help="Server<-->worker websocket URI")
    ...
    global SILENCE_TIMEOUT
    SILENCE_TIMEOUT = conf.get("silence-timeout", 10)
    if USE_NNET2:
    ...
    main_loop(args.uri, decoder_pipeline, post_processor,
              full_post_processor)    # so that can auto exit
