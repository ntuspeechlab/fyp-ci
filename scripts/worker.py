...
class ServerWebsocket(worker_addon.BaseServerWebsocket):
    ...
    def received_message(self, m):
        ...
        if self.state == self.__class__.STATE_CONNECTED:
            ...
            logger.info("%s: User sending I-AM-ALIVE message." % (self.request_id))
    ...
    def _on_full_result(self, full_result_json):
        try:
            ...
            if full_result.get("status", -1) == common.STATUS_SUCCESS:
                ret_full_result = ""
                try:
                    ret_full_result = repr(full_result).decode("unicode-escape")
                except:
                    ret_full_result = repr(full_result)

                logger.debug(u"%s: Before postprocessing: %s" % (self.request_id, ret_full_result))
                try: 
                ...
                logger.info("%s: Postprocessing done." % self.request_id)
                logger.debug(u"%s: After postprocessing: %s" % (self.request_id, ret_full_result))
    ...
    def send_adaptation_state(self):
        if hasattr(self.decoder_pipeline, 'get_adaptation_state'):
            ...
            if sys.version_info[0] < 3:
                adaptationvalue=base64.b64encode(zlib.compress(adaptation_state))
            else:
                adaptationvalue=base64.b64encode(zlib.compress(adaptation_state.encode()))
        ...
        if self.post_processor:
            if self.post_processor_lock.acquire(blocking):
                ...
                for text in texts:
                    ...
                    logger.debug("%s: Starting postprocessing: %s"  % (self.request_id, text))
                    text = yield self.post_processor.stdout.read_until(b'\n')
                    text = text.decode("utf-8")
                    text = text.strip()
                    text = text.replace("\\n", "\n")
                    logger.debug("%s: Postprocessing returned: %s"  % (self.request_id, text))
                    result.append(text)
    ...
    def post_process_full(self, full_result):
        if self.full_post_processor:
            ...
            while True:
                ...
                logger.debug("%s: full_post_processor returned: %s"  % (self.request_id, nl))
                if nl.strip() == "()":
                    break
                if nl.strip() == "":
                    break
