...
class BaseServerWebsocket(WebSocketClient):
    ...
    def guard_idle_run_once_worker(self):
        while self.state == self.STATE_CONNECTED:
            if time.time() - self.start_time > IDLE_TIMEOUT:
                logging.warning("%s: Worker has more than %d seconds idle, shutting down" % (
                    self.request_id, IDLE_TIMEOUT))
                self.finish_request()
                self.close()
                return
            time.sleep(1)
    ...
    def monitor_idle_run_once_worker(self):
        if os.getenv('RUN_FREQ', False) == 'ONCE':
            logging.info(
                'start monitor idle RUN_FREQ=ONCE worker for more than {} seconds'.format(IDLE_TIMEOUT))
